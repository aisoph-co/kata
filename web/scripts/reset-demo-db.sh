#!/usr/bin/env bash
# Resets Postgres + reseeds + restarts the API only (leaves the web dev
# server alone — Playwright's own webServer manages that). Used by
# e2e-loop.sh between runs so each run starts from the same, fully
# populated Dee/Ada state rather than whatever the previous run's real
# submissions left behind (state now persists for real in Postgres).
set -euo pipefail
