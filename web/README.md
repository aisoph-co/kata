...
in a deployment that has the two Auth0 values configured — pair it with
`AUTH0_DISABLED` (below) or the persona switcher's writes stay reachable
by anyone, gate or no gate.
...
| `VITE_AUTH0_DISABLED` | build-time only | Sign-in gate kill switch (KATA-28). Unset/anything but `true` (default) — no effect, gate follows the two vars above. `true` — force the gate off even if a tenant is configured. UI-only — pair with `AUTH0_DISABLED` below |
| `AUTH0_DISABLED` | container env (prod, Caddyfile) / `web/.env` (dev) | Read-only enforcement (KATA-28), the runtime companion to `VITE_AUTH0_DISABLED`. `true` — blocks every POST/PUT/PATCH/DELETE to `/api/*` except `/identities/resolve` (a lookup, never a write), since the persona switcher's `X-Acting-Identity` is client-controlled regardless of the gate. Unset (default) — no effect |
