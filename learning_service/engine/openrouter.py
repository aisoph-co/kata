"""OpenRouter-backed grader for `short_answer` items (spec §Learning
engine; §API "Grading": "the service grades, calling OpenRouter with a
per-item rubric").

`LLMClient` is a structural protocol so `grading.grade_response` — and the
test suite — can swap in a stub with no live model calls (US-B2b's AC).
"""
...
class OpenRouterClient:
    def __init__(self, api_key: str | None = None, model: str | None = None, timeout: float = 20.0) -> None:
        ...
        # `model=` still wins when a caller passes one explicitly (tests do);
        # otherwise `OPENROUTER_MODEL` is read at construction time, falling
        # back to the unchanged `DEFAULT_MODEL`.
